...
const blockOptions: BlockOption[] = [
  { value: "paragraph", label: "텍스트", shortcut: "text", aliases: ["text", "paragraph", "텍스트", "본문"], icon: "T", description: "일반 텍스트 블록" },
  { value: "heading1", label: "제목 1", shortcut: "h1", aliases: ["h1", "heading1", "heading 1", "제목1", "제목 1"], icon: "H1", description: "큰 섹션 제목", markdownHint: "# + Space" },
  { value: "heading2", label: "제목 2", shortcut: "h2", aliases: ["h2", "heading2", "heading 2", "제목2", "제목 2"], icon: "H2", description: "중간 섹션 제목", markdownHint: "## + Space" },
  { value: "heading3", label: "제목 3", shortcut: "h3", aliases: ["h3", "heading3", "heading 3", "제목3", "제목 3"], icon: "H3", description: "작은 섹션 제목", markdownHint: "### + Space" },
  { value: "bullet", label: "글머리 기호", shortcut: "bullet", aliases: ["bullet", "bulleted", "bulleted list", "list", "글머리", "목록"], icon: "•", description: "글머리 목록", markdownHint: "- + Space" },
  { value: "number", label: "번호 목록", shortcut: "number", aliases: ["number", "numbered", "numbered list", "ordered", "번호", "번호목록"], icon: "1.", description: "번호가 있는 목록", markdownHint: "1. + Space" },
  { value: "todo", label: "할 일", shortcut: "todo", aliases: ["todo", "to-do", "check", "checkbox", "checklist", "할일", "체크"], icon: "☐", description: "체크 가능한 할 일", markdownHint: "[ ] + Space" },
  { value: "quote", label: "인용", shortcut: "quote", aliases: ["quote", "blockquote", "callout", "인용", "콜아웃"], icon: "❝", description: "인용/강조 블록", markdownHint: "> + Space" },
  { value: "code", label: "코드", shortcut: "code", aliases: ["code", "code block", "코드", "코드블록"], icon: "</>", description: "코드 블록", markdownHint: "``` + Space" },
  { value: "math", label: "수식", shortcut: "equation", aliases: ["equation", "math", "latex", "수식", "공식"], icon: "∑", description: "LaTeX 수식 블록", markdownHint: "$$ + Space" },
  { value: "divider", label: "구분선", shortcut: "divider", aliases: ["divider", "line", "separator", "구분선", "선"], icon: "—", description: "섹션 구분선", markdownHint: "--- + Space" },
];
...
export function NotionNoteEditor({ value, areas, onChange }: {
  ...
  function convertBlock(index: number, type: StudyNoteBlockType, clearText = false) {
    const block = blocks[index];
    if (!block) return;
    const patch: Partial<StudyNoteBlock> = {
      type,
      text: type === "divider" || clearText ? "" : block.text,
  ...
  return <div className="scrollbar h-full overflow-y-auto bg-[#151516] px-3 py-5 sm:px-8 lg:px-12">
    <div className="mx-auto max-w-[820px] pb-28">
      <div className="mb-6 flex items-center justify-between gap-3 px-2 text-[11px] text-[var(--muted)]">
        <span>Notion식 입력 · #/##/###, -, 1., &gt;, [ ], ```, $$ 뒤 Space · / 명령 · 블록 자체 드래그</span>
        ...
        {blocks.map((block, index) => {
          ...
          >
            ...
            /> : <RichTextBlock
              ...
              onConvert={(type) => convertBlock(index, type, true)}
              ...
              onEmptyBackspace={() => {
                if (block.type !== "paragraph") patchBlock(index, { type: "paragraph", checked: undefined });
...
function RichTextBlock({ block, active, onActivate, onChange, onToggleTodo, onConvert, onSplit, onEmptyBackspace }: {
  ...
  onToggleTodo: () => void;
  onConvert: (type: StudyNoteBlockType) => void;
  ...
  const slashOptions = slashQuery === null ? [] : getSlashOptions(slashQuery);
  ...
  function applyInlineFormattingAtEnd(markdown: string) {
    const node = ref.current;
    if (!node) return;
    const next = `${markdown} `;
    onChange(next);
  ...
  return <div className="relative py-0.5" onClick={onActivate}>
      ...
      onKeyDown={(event) => {
        ...
        if (slashQuery !== null && slashOptions.length > 0) {
          ...
          if (event.key === "Enter" && !event.shiftKey) {
            event.preventDefault();
            const exact = findExactSlashOption(slashQuery);
            onConvert(exact?.value ?? slashOptions[slashIndex]?.value ?? slashOptions[0]!.value);
            return;
          }
        }
        ...
        if (event.key === " " && isCaretAtEnd(node)) {
          ...
          if (hasCompletedInlineMarkdown(markdown)) {
            event.preventDefault();
            applyInlineFormattingAtEnd(markdown);
            return;
          }
        }
    ...
    {slashQuery !== null && slashOptions.length > 0 && <SlashMenu
      options={slashOptions}
      selectedIndex={slashIndex}
      onSelect={(type) => onConvert(type)}
    />}
  </div>;
}
...
function SourceBlock({ block, active, onActivate, onChange, onLanguageChange }: {
  ...
  return <div className="py-1">
    ...
    {block.type === "code" && <input
      type="text"
      value={block.language ?? ""}
      onChange={(event) => onLanguageChange(event.target.value)}
      placeholder="언어 (예: python, javascript)"
      aria-label="코드 언어"
      className="mb-1.5 w-44 rounded-md border border-white/8 bg-black/20 px-2 py-1 text-[11px] text-[var(--muted)] outline-none focus:border-white/15"
    />}
    ...
    {options.length > 0 && <>
      ...
      <div className="max-h-64 overflow-y-auto">
        {options.map((option) => <button key={option.value} type="button" onClick={() => onType(option.value)} className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left hover:bg-white/[.07]">
          ...
          <span className="min-w-0 flex-1"><span className="block text-xs text-white">{option.label}</span><span className="block truncate text-[10px] text-[var(--muted)]">{option.description}</span></span>
          {option.markdownHint && <span className="shrink-0 text-[9px] text-white/30">{option.markdownHint}</span>}
        </button>)}
      </div>
...
function SlashMenu({ options, selectedIndex, onSelect }: {
  ...
  onSelect: (type: StudyNoteBlockType) => void;
}) {
  return <div className="absolute left-4 top-full z-40 w-80 rounded-xl border border-white/10 bg-[#252527] p-1.5 shadow-2xl">
    <p className="px-2 py-1 text-[10px] font-medium uppercase tracking-[.08em] text-[var(--muted)]">블록 명령 · ↑↓ 선택 · Enter 적용</p>
...
function getSlashOptions(query: string): BlockOption[] {
  const normalized = query.trim().toLowerCase();
  if (!normalized) return blockOptions;
  return blockOptions.filter((option) => {
    const haystack = [option.label, option.shortcut, option.description, ...option.aliases].join(" ").toLowerCase();
    return haystack.includes(normalized);
  });
}

function findExactSlashOption(query: string): BlockOption | undefined {
  const normalized = query.trim().toLowerCase();
  return blockOptions.find((option) => option.shortcut.toLowerCase() === normalized || option.aliases.some((alias) => alias.toLowerCase() === normalized));
}
