...
export function StudyChat({
  ...
  return <aside className="flex min-h-[520px] flex-col border-t border-[var(--line)] bg-black lg:min-h-0 lg:border-l lg:border-t-0" aria-label="학습 대화">
    ...
    <div className="scrollbar flex-1 space-y-3 overflow-y-auto p-4">{messages.length === 0 && <div className="rounded-xl border border-[var(--line)] bg-[#111] p-4 text-sm leading-relaxed text-[#bbb]">Gemini API가 현재 페이지와 형광펜·밑줄·영역으로 표시한 질문 문맥, 최근 대화를 사용해 답합니다. 수식·그림·표는 영역 도구로 사각형 선택하면 이미지 자체가 질문에 전달됩니다.</div>}{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`rounded-xl p-3 text-sm leading-relaxed ${message.role === "user" ? "ml-7 bg-white text-black" : "mr-7 border border-[var(--line)] bg-[#111]"}`}><p className={`mb-1 text-[10px] uppercase tracking-wider ${message.role === "user" ? "text-[#555]" : "text-[var(--muted)]"}`}>{message.role === "user" ? "You" : "Gemini"}</p><p className="whitespace-pre-wrap">{message.content}</p>{message.role === "assistant" && messages[index - 1]?.role === "user" && <button onClick={() => onSaveInsight(messages[index - 1].content, message.content)} className="mt-3 rounded border border-[var(--line)] px-2 py-1 text-xs hover:bg-white hover:text-black">Save Insight</button>}</div>)}{loading && <p role="status" className="text-sm text-[var(--muted)]">답변을 생성하는 중…</p>}{error && <p role="alert" className="rounded-lg border border-white p-3 text-sm">{error}</p>}</div>
