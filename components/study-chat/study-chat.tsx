...
export function StudyChat({ paper, context, onClearSelection }: { paper: Paper; context: StudyContext; onClearSelection: () => void }) {
  ...
  return <aside className="flex min-h-[520px] flex-col border-t border-[var(--line)] bg-[#0d1210] lg:min-h-0 lg:border-l lg:border-t-0" aria-label="학습 대화">
    ...
    <div className="scrollbar flex-1 space-y-3 overflow-y-auto p-4">{messages.length === 0 && <div className="rounded-xl border border-[#26312c] bg-[#131916] p-4 text-sm leading-relaxed text-[#aab4af]">OpenAI API가 현재 페이지, 선택 문장과 최근 대화만 사용해 답합니다. API 사용은 ChatGPT Plus와 별도로 과금됩니다.</div>}{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`rounded-xl p-3 text-sm leading-relaxed ${message.role === "user" ? "ml-7 bg-[#1b2a21]" : "mr-7 border border-[#26312c] bg-[#131916]"}`}><p className="mb-1 text-[10px] uppercase tracking-wider text-[var(--muted)]">{message.role === "user" ? "You" : "AI"}</p><p className="whitespace-pre-wrap">{message.content}</p></div>)}{loading && <p role="status" className="text-sm text-[var(--muted)]">답변을 생성하는 중…</p>}{error && <p role="alert" className="rounded-lg bg-red-950/40 p-3 text-sm text-red-200">{error}</p>}</div>
