...
export function StudyChat({
  ...
  return <aside className="flex min-h-[520px] flex-col border-t border-[var(--line)] bg-black lg:min-h-0 lg:border-l lg:border-t-0" aria-label="학습 대화">
    ...
    <div className="scrollbar flex-1 space-y-3 overflow-y-auto p-4">
      {messages.length === 0 && <div className="rounded-xl border border-[var(--line)] bg-[#111] p-4 text-sm leading-relaxed text-[#bbb]">Gemini API가 현재 페이지와 형광펜·밑줄·영역으로 표시한 질문 문맥, 최근 대화를 사용해 답합니다. 답변은 Markdown과 수식 문법을 렌더링해서 보여줍니다.</div>}
      {messages.map((message, index) => <div key={`${message.role}-${index}`} className={`rounded-2xl p-3.5 text-sm leading-relaxed ${message.role === "user" ? "ml-7 bg-[var(--accent)] text-white" : "mr-7 border border-[var(--line)] bg-[rgba(255,255,255,.045)]"}`}>
        ...
        {message.role === "assistant" && messages[index - 1]?.role === "user" && <button onClick={() => onSaveInsight(messages[index - 1].content, message.content)} className="mt-3 rounded-lg border border-[var(--line)] px-2.5 py-1.5 text-xs hover:bg-white/10">Save Insight</button>}
      </div>)}
