...
export function StudyChat({ paper, context, onClearSelection }: { paper: Paper; context: StudyContext; onClearSelection: () => void }) {
  ...
  return <aside className="flex min-h-[520px] flex-col border-t border-[var(--line)] bg-[#0d1210] lg:min-h-0 lg:border-l lg:border-t-0" aria-label="학습 대화">
    ...
    <div className="scrollbar flex-1 space-y-3 overflow-y-auto p-4">{messages.length === 0 && <div className="rounded-xl border border-[#26312c] bg-[#131916] p-4 text-sm leading-relaxed text-[#aab4af]">Gemini API가 현재 페이지, 선택 문장과 최근 대화만 사용해 답합니다. 사용량과 과금은 연결한 Google AI 프로젝트 정책을 따릅니다.</div>}{messages.map((message, index) => <div key={`${message.role}-${index}`} className={`rounded-xl p-3 text-sm leading-relaxed ${message.role === "user" ? "ml-7 bg-[#1b2a21]" : "mr-7 border border-[#26312c] bg-[#131916]"}`}><p className="mb-1 text-[10px] uppercase tracking-wider text-[var(--muted)]">{message.role === "user" ? "You" : "Gemini"}</p><p className="whitespace-pre-wrap">{message.content}</p></div>)}{loading && <p role="status" className="text-sm text-[var(--muted)]">답변을 생성하는 중…</p>}{error && <p role="alert" className="rounded-lg bg-red-950/40 p-3 text-sm text-red-200">{error}</p>}</div>
    <form onSubmit={submit} className="border-t border-[var(--line)] p-4">{context.selectedText && <div className="mb-2 rounded-lg bg-[#19251f] p-2 text-xs"><div className="flex justify-between"><span className="text-[var(--accent)]">선택 텍스트 · p.{context.page}</span><button type="button" onClick={onClearSelection}>제거</button></div><p className="mt-1 line-clamp-3 text-[#b7c0bc]">{context.selectedText}</p></div>}<label htmlFor="chat" className="sr-only">질문</label><textarea id="chat" maxLength={4000} rows={3} value={input} onChange={(event) => setInput(event.target.value)} placeholder={`${paper.title}에 관해 질문하세요…`} className="w-full resize-none rounded-xl border border-[var(--line)] bg-[#151b18] p-3 text-sm"/><div className="mt-2 flex items-center justify-between"><span className="text-xs text-[var(--muted)]">문맥: p.{context.page}</span><button disabled={loading || !input.trim()} className="rounded-lg bg-[#b2d9be] px-4 py-2 text-sm font-medium text-[#0c120e] disabled:opacity-40">질문하기</button></div></form>
  </aside>;
}
