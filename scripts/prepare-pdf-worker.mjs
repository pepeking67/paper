...
const pdfiumWasmSource = path.join(path.dirname(pdfiumEntry), "pdfium.wasm");
const publicDirectory = path.resolve("public");
const workerDestination = path.join(publicDirectory, "pdf.worker.min.mjs");
const pdfiumWasmDestination = path.join(publicDirectory, "pdfium.wasm");
...
await cp(pdfiumWasmSource, pdfiumWasmDestination);
...
console.log("Prepared PDF.js resources and PDFium WASM in public/");
