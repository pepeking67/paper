...
With `SITE_PASSWORD` and `BLOB_READ_WRITE_TOKEN` configured, open the deployed site,
authenticate as user `paper`, and choose **PDF 관리** at the bottom left. **누락된
...
checked-in manifest, enforces same-origin Basic authentication, download timeout,
