...
test("admin sync authentication requires configured valid Basic auth and same origin", () => {
  const notConfigured = verifyAdminRequest(new Request("https://paper.test/api/admin/sync-pdfs", { method: "POST" }), undefined);
  assert.equal(notConfigured.ok, false);
  if (!notConfigured.ok) assert.equal(notConfigured.status, 503);
  const unauthorized = verifyAdminRequest(new Request("https://paper.test/api/admin/sync-pdfs", { method: "POST" }), "secret");
  assert.equal(unauthorized.ok, false);
  const request = new Request("https://paper.test/api/admin/sync-pdfs", { method: "POST", headers: { authorization: `Basic ${btoa("paper:secret")}`, origin: "https://paper.test" } });
  assert.deepEqual(verifyAdminRequest(request, "secret"), { ok: true });
});
